    ...
    return `
      <a href="producto.html?id=${p.id}" class="product-card">
        ...
        <div style="margin-top:0.75rem;">
          ...
          <p style="font-family:'Syne',sans-serif;font-size:0.875rem;font-weight:bold;margin:0.25rem 0 0;">S/${p.price.toFixed(2)}</p>
          <div style="display:flex;align-items:center;gap:0.375rem;margin-top:0.5rem;">
            ${p.colors.map(c => `<span title="${c.name}" style="width:1rem;height:1rem;border-radius:9999px;border:1px solid var(--border);background:${c.hex};"></span>`).join('')}
          </div>
          <p style="font-size:0.625rem;color:var(--muted-foreground);text-transform:uppercase;letter-spacing:0.1em;margin:0.25rem 0 0;">${p.sizes.join(' · ')}</p>
        </div>
      </a>`;
  }
    ...
    if (!p) {
      host.innerHTML = `
        <div class="section-padding" style="text-align:center;">
          <h1 class="font-heading" style="font-size:2.25rem;font-weight:800;margin-bottom:1rem;">Producto no encontrado</h1>
