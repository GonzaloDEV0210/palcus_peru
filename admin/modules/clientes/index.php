...
$params = [];
if ($search) {
    $where[] = '(nombre LIKE ? OR dni_ruc LIKE ? OR email LIKE ? OR telefono LIKE ?)';
    $term = "%$search%";
    $params = array_fill(0, 4, $term);
}
$whereSQL = 'WHERE ' . implode(' AND ', $where);

$total = (int)db()->fetchOne("SELECT COUNT(*) AS n FROM clientes $whereSQL", $params)['n'];
$pages = max(1, (int)ceil($total / $perPage));
$offset = ($page - 1) * $perPage;

$clientes = db()->fetchAll(
    "SELECT * FROM clientes $whereSQL ORDER BY nombre ASC LIMIT $perPage OFFSET $offset",
    $params
);
?>
<!DOCTYPE html>
    ...
    <main class="flex-1 p-6 space-y-5">
      ...
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p class="text-gray-400 text-xs mt-0.5"><?= $total ?> clientes registrados</p>
        </div>
      ...
      <form method="GET" class="flex flex-wrap gap-3">
        <div class="relative flex-1 min-w-52">
          ...
          <input name="q" value="<?= e($search) ?>" placeholder="Buscar por nombre, DNI/RUC, email o teléfono…" class="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 rounded-xl bg-white focus:outline-none focus:border-gray-400"/>
        </div>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="bg-gray-50 text-gray-500 text-xs font-semibold uppercase tracking-wide">
                ...
                <th class="px-5 py-3 text-left">DNI / RUC</th>
            ...
            <tbody class="divide-y divide-gray-100">
              ...
              <tr><td colspan="5" class="text-center py-12 text-gray-400">No se encontraron clientes.</td></tr>
              <?php else: ?>
              <?php foreach ($clientes as $c): ?>
              <tr class="hover:bg-gray-50 transition-colors">
                <td class="px-5 py-3">
                  <p class="font-medium text-gray-900"><?= e($c['nombre']) ?></p>
                  <p class="text-xs text-gray-400"><?= e($c['email'] ?: 'Sin correo') ?></p>
                </td>
                <td class="px-5 py-3 font-mono text-xs text-gray-600"><?= e($c['dni_ruc'] ?: '—') ?></td>
                <td class="px-5 py-3 text-gray-600">
                  <p><?= e($c['telefono'] ?: '—') ?></p>
                </td>
                <td class="px-5 py-3 text-gray-600 max-w-xs truncate"><?= e($c['direccion'] ?: '—') ?></td>
                <td class="px-5 py-3">
                  <div class="flex items-center justify-center gap-1">
                    ...
                    <form method="POST" action="eliminar.php" data-confirm="¿Deseas eliminar este cliente permanentemente?">
