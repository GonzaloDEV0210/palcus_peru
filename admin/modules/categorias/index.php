        ...
        <div class="<?= $level > 0 ? 'tree-node' : '' ?>">
          <div class="cat-row <?= $inactive ?>">
            ...
            <div class="flex items-center gap-2 shrink-0">
              <!-- Editar Categoría -->
              <button onclick='openModal(<?= json_encode(['id'=>(int)$node["id"],'nombre'=>$node["nombre"],'prefijo'=>$node["prefijo"],'parent_id'=>$node["parent_id"],'descripcion'=>$node["descripcion"],'orden'=>(int)$node["orden"],'activo'=>(int)$node["activo"]]) ?>)' class="w-8 h-8 rounded-xl flex items-center justify-center bg-blue-50 text-blue-600 border border-blue-200 shadow-sm hover:bg-blue-100 hover:text-blue-700 hover:scale-110 active:scale-95 transition-all duration-200" title="Editar Categoría">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                </svg>
              </button>
              <!-- Eliminar Categoría -->
              <form method="POST" onsubmit="return confirm('¿Eliminar «<?= e($node["nombre"]) ?>»?')" class="inline">
                <input type="hidden" name="action" value="delete"/>
                <input type="hidden" name="id" value="<?= $node['id'] ?>"/>
                <button type="submit" class="w-8 h-8 rounded-xl flex items-center justify-center bg-red-50 text-red-500 border border-red-200 shadow-sm hover:bg-red-100 hover:text-red-600 hover:scale-110 active:scale-95 transition-all duration-200" title="Eliminar Categoría">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
